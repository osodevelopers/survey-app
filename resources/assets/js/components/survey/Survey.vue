  ...
  export default {
    ...
    methods: {
      ...
      getUserRole(){
        ...
        axios.get('api/user-role',{ headers }).then(response => {
          console.log(response);
          this.$store.dispatch('setRole', response.data[0].slug);
          window.localStorage.setItem('role',JSON.stringify(response.data[0].slug));
        });
      },
      getSurveys(){
        let headers = this.header;
